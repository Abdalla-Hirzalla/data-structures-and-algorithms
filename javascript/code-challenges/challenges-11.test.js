...
const count = (target, input) => {
  const count = input.map(arr => arr.reduce((acc, cur) => {
    if (cur === target) {
      acc++;
    }
    return acc;
  }, 0));
  return count.reduce((cur, acc) => acc += cur, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const divisibleByFive = input.map(arr => arr.filter(arr => arr % 5 === 0 && Number.isInteger(arr)));
  const power = divisibleByFive.map(arr => arr.reduce((acc, cur) => {
...
let findMaleAndFemale = (data) => {
  ...
  const names = characters.reduce((acc, cur) => {
    acc.push(cur.name);
    return acc;
  }, [])
