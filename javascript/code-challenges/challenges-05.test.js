...
const toLastNames = people => {
  let array = [];
  people.map((word) => {
    array.push(word.firstName + ' ' + word.lastName);
  });
  return array;
};
...
const addPurchases = (arr) => {
  let total = arr.reduce((sum, item) =>{return sum+=item.purchasePrice;},0);
  return total;
};
...
const countNumberOfElements = (arr) => {
  let total = arr.reduce((sum) => {return sum+=1;},0);
  return total;
};
...
const returnNames = (arr) => {
  const characters = arr.reduce((stars, actor, i) => {
    stars[i] = actor.name;
    return stars;
  },[]);
  return characters;
